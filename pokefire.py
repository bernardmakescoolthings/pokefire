...
from modules.monitor_lock import MonitorLock
...
def load_config(path, source="ebay"):
    ...
    if source == "ebay":
        build_query(config)
    from modules.discord_bot import validate as validate_discord
...
from modules.scrapingdog import ScrapingdogClient as EbayClient
...
def main():
    ...
    parser.add_argument("--env-file", help="Credentials file (default: .env next to pokefire.py)")
    ...
    monitor_lock = None
    try:
        load_env(args.env_file or Path(__file__).with_name(".env"), required=args.env_file is not None)
        ...
        if args.export:
            ...
            return 0
        from modules.discord_bot import notifier
